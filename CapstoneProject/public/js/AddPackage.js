 //Global Variables
    ...
    function ShowPackageTables(){
        document.getElementById("packageTables").style.display = "block";
    }

    function HidePackageTables(){
        document.getElementById("packageTables").style.display = "none";
    }
    ...
    function CheckPackageForm(){
        ...
        var PackagePax = document.getElementById("PackagePax").value;

        if((PackageID == "") || (PackageName == "") || (PackagePax == "") || (PackageDuration == "")){
            var x = document.getElementsByClassName("ErrorLabel");
            for(var i = 0; i < x.length; i++){
                x[i].innerText="Please fill out all fields with *";
                FormError = true;
            }
        }
    ...
    function RemoveRoomType(){
        var TableChecker = CheckTable('#PacRoomTable tr');
            if(TableChecker){
                TotalAmount = parseInt(TotalAmount) - (parseInt(RoomInfo[2]) * parseInt(document.getElementById("PackageDuration").value));
                document.getElementById("TotalAmount").innerHTML = TotalAmount;
                ...
                select.add(option);
                var tempRoomCapacity = strRoomCapacity.split(",");
                for(var x = 0; x < tempRoomCapacity.length; x++){
                    var arrRoomCapacity = tempRoomCapacity[x].split("-");
                    if(arrRoomCapacity[0] == RoomInfo[0]){
                        TotalRoomCapacity -= parseInt(arrRoomCapacity[1]);
                        strRoomCapacity = strRoomCapacity.replace(arrRoomCapacity[0]+"-"+arrRoomCapacity[1]+",",'');
                        document.getElementById("TotalRoomCapacity").innerHTML = TotalRoomCapacity;
                        break;
                    }
                }
    ...
    function PackagePaxEvent(field, dataType, holder){
        document.getElementById("i-PackagePax").innerHTML = document.getElementById("PackagePax").value;
        RoomCapacityChecker();
    ...
    function RoomCapacityChecker(){
        if(TotalRoomCapacity >= parseInt(document.getElementById("i-PackagePax").innerHTML)){
            ...
            document.getElementById("PaxError").innerHTML = "Package pax exceeds total room capacity";
        }
    }
        ...
        if(document.getElementById("PackagePrice").value != ""){
            ...
            $("#PacRoomTable tr").each(function(){
                if(($(this).find("td:nth-child(3)").text())!=""){
                    OldTotal += parseInt($(this).find("td:nth-child(3)").text()) * parseInt(document.getElementById("PackageDuration").value);   
                }
            ...
            document.getElementById("TotalAmount").innerHTML = TotalAmount;
            document.getElementById("i-PackagePax").innerHTML = document.getElementById("PackagePax").value;
            document.getElementById("TotalRoomCapacity").innerHTML = localStorage.getItem("TotalRoomCapacity");
            TotalRoomCapacity = localStorage.getItem("TotalRoomCapacity");
            localStorage.removeItem("TotalRoomCapacity");
            } 
    }
    ...
    function SavePackage(){
        ...
        if($(".form-group").hasClass("has-warning")){  
            ...
            localStorage.setItem("TotalRoomCapacity", document.getElementById("TotalRoomCapacity").innerHTML);
            return true;
        }   
    }
